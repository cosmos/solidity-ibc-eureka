...
func (s *IbcEurekaSolanaTestSuite) SetupIBCInfrastructure(ctx context.Context) {
	...
	// Initialize router first (required before GMP can register)
...
func (s *IbcEurekaSolanaTestSuite) SetupTransferApp(ctx context.Context) {
	s.Require().True(s.Run("Deploy and Register Dummy App", func() {
		...
		appStateAccount, _, err := solanago.FindProgramAddress([][]byte{[]byte("app_state"), []byte(TransferPortID)}, dummyAppProgramID)
		s.Require().NoError(err)
		...
		ibcAppAccount, _, err := solanago.FindProgramAddress([][]byte{[]byte("ibc_app"), []byte(TransferPortID)}, ics26_router.ProgramID)
		s.Require().NoError(err)
...
func (s *IbcEurekaSolanaTestSuite) Test_SolanaToCosmosTransfer_SendPacket() {
	...
	s.Require().True(s.Run("Relay acknowledgment back to Cosmos", func() {
		...
		s.Require().True(s.Run("Broadcast acknowledgment relay tx on Cosmos", func() {
			...
			var err error
			cosmosPacketRelayTxHash, err = hex.DecodeString(relayTxResult.TxHash)
			s.Require().NoError(err)
	...
	s.Require().True(s.Run("Acknowledge packet on Solana", func() {
		s.Require().True(s.Run("Update Tendermint client on Solana via chunks", func() {
			...
			s.submitUpdateClient(ctx, resp, s.SolanaUser)
			s.Require().NoError(err, "Failed to submit update client transactions")
		...
		s.Require().True(s.Run("Relay acknowledgment", func() {
			...
			s.Require().NoError(err)
			s.Require().NotEmpty(resp.Tx, "Acknowledgement transaction should not be empty")

			unsignedSolanaTx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(resp.Tx))
			s.Require().NoError(err)
			s.T().Logf("Acknowledgement transaction contains %d instructions", len(unsignedSolanaTx.Message.Instructions))

			sig, err := s.SolanaChain.SignAndBroadcastTxWithRetry(ctx, unsignedSolanaTx, s.SolanaUser)
			s.Require().NoError(err, "Acknowledgement tx failed")
...
func (s *IbcEurekaSolanaTestSuite) Test_SolanaToCosmosTransfer_SendTransfer() {
	...
	s.Require().True(s.Run("Relay transfer to Cosmos", func() {
		...
		s.Require().True(s.Run("Broadcast relay tx on Cosmos", func() {
			relayTxResult := s.MustBroadcastSdkTxBody(ctx, simd, s.CosmosUsers[0], CosmosDefaultGasLimit, relayTxBodyBz)
			s.T().Logf("Relay transaction: %s (code: %d, gas: %d)",
				relayTxResult.TxHash, relayTxResult.Code, relayTxResult.GasUsed)

			var err error
			cosmosRelayTxHash, err = hex.DecodeString(relayTxResult.TxHash)
			s.Require().NoError(err)
		}))
	}))
...
func (s *IbcEurekaSolanaTestSuite) Test_CosmosToSolanaTransfer() {
	...
	s.Require().True(s.Run("Send ICS20 transfer from Cosmos to Solana", func() {
		...
		s.Require().True(s.Run("Send transfer packet from Cosmos", func() {
			...
			payload := channeltypesv2.Payload{
				SourcePort:      TransferPortID,
				DestinationPort: TransferPortID,
				Version:         transfertypes.V1,
	...
	s.Require().True(s.Run("Relay packet from Cosmos to Solana", func() {
		...
		s.Require().True(s.Run("Broadcast relay tx on Solana", func() {
	...
	s.Require().True(s.Run("Verify packet received on Solana", func() {
		...
		dummyAppStateAccount, _, err := solanago.FindProgramAddress([][]byte{[]byte("app_state"), []byte(TransferPortID)}, s.DummyAppProgramID)
		s.Require().NoError(err)
...
func (s *IbcEurekaSolanaTestSuite) submitUpdateClient(ctx context.Context, resp *relayertypes.UpdateClientResponse, user *solanago.Wallet) {
	s.Require().NotEqual(0, len(resp.Txs), "no transactions provided")

	if len(resp.Txs) == 1 {
		s.submitSingleTransaction(ctx, resp.Txs[0], user)
		return
	}

	s.submitChunkedUpdateClient(ctx, resp.Txs, user)
}

func (s *IbcEurekaSolanaTestSuite) submitSingleTransaction(ctx context.Context, txBytes []byte, user *solanago.Wallet) {
	s.T().Log("Submitting single update client transaction (mock mode)")

	tx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(txBytes))
	s.Require().NoError(err, "Failed to decode single update client tx")
	...
	s.Require().NoError(err)
	s.T().Logf("Single update client transaction submitted: %s", sig)
}

func (s *IbcEurekaSolanaTestSuite) submitChunkedUpdateClient(ctx context.Context, txs [][]byte, user *solanago.Wallet) {
	chunkCount := len(txs) - 2
	s.T().Logf("Submitting %d transactions: 1 metadata + %d chunks (parallel) + 1 assembly", len(txs), chunkCount)

	s.submitMetadataTransaction(ctx, txs[0], user)
	s.submitChunksInParallel(ctx, txs[1:len(txs)-1], user)
	s.submitAssemblyTransaction(ctx, txs[len(txs)-1], user)

	s.T().Logf("Successfully submitted all %d chunked transactions", len(txs))
}

func (s *IbcEurekaSolanaTestSuite) submitMetadataTransaction(ctx context.Context, txBytes []byte, user *solanago.Wallet) {
	tx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(txBytes))
...
func (s *IbcEurekaSolanaTestSuite) submitChunksInParallel(ctx context.Context, chunkTxs [][]byte, user *solanago.Wallet) {
	...
	chunkResults := make(chan chunkResult, len(chunkTxs))

	for i, txBytes := range chunkTxs {
		go func(idx int, txData []byte) {
			tx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(txData))
			if err != nil {
				chunkResults <- chunkResult{index: idx, err: fmt.Errorf("failed to decode chunk %d: %w", idx, err)}
				return
			}

			sig, err := s.SolanaChain.SignAndBroadcastTxWithRetry(ctx, tx, user)
			if err != nil {
				chunkResults <- chunkResult{index: idx, err: fmt.Errorf("failed to submit chunk %d: %w", idx, err)}
				return
			}
			chunkResults <- chunkResult{index: idx, sig: sig}
		}(i, txBytes)
	}

	for i := 0; i < len(chunkTxs); i++ {
		result := <-chunkResults
...
func (s *IbcEurekaSolanaTestSuite) submitAssemblyTransaction(ctx context.Context, txBytes []byte, user *solanago.Wallet) {
	tx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(txBytes))
...
func (s *IbcEurekaSolanaTestSuite) verifyAcknowledgmentOnSolana(ctx context.Context, clientID string, sequence uint64) {
	// When an acknowledgement is processed on Solana, the router closes the packet_commitment account
	// So we verify that the packet_commitment PDA no longer exists or has been zeroed
	...
	if err != nil {
		// Account not found is expected - acknowledgement was processed and commitment was closed
		s.T().Logf("Packet commitment account not found (expected) - acknowledgement was processed")
		return
	}

	// If account exists, verify it was closed (0 lamports)
	if accountInfo.Value != nil {
		s.Require().Equal(uint64(0), accountInfo.Value.Lamports,
			"Packet commitment should be closed (0 lamports) after acknowledgement is processed")
	}

	s.T().Logf("Acknowledgment verified on Solana for client %s, sequence %d", clientID, sequence)
	s.T().Logf("  - Packet commitment account was closed: %s", packetCommitmentPDA.String())
}

func getSolDenomOnCosmos() transfertypes.Denom {
	return transfertypes.NewDenom(SolDenom, transfertypes.NewHop(TransferPortID, CosmosClientID))
}
...
func (s *IbcEurekaSolanaTestSuite) prepareBaseAccounts(ctx context.Context, dummyAppProgramID solanago.PublicKey, port, clientID string) AccountSet {
	...
	accounts.AppState, _, err = solanago.FindProgramAddress([][]byte{[]byte("app_state"), []byte(TransferPortID)}, dummyAppProgramID)
	s.Require().NoError(err)
	...
	sequenceBytes := solana.Uint64ToLeBytes(nextSequence)
...
func (s *IbcEurekaSolanaTestSuite) Test_GMPCounterFromCosmos() {
	...
	s.Require().True(s.Run("Setup User Identities and Helpers", func() {
		...
		relayGMPPacket = func(cosmosGMPTxHash []byte, userLabel string) solanago.Signature {
			...
			solanaRelayTxSig, err = s.SolanaChain.SignAndBroadcastTxWithRetry(ctx, unsignedSolanaTx, s.SolanaUser)
			s.Require().NoError(err)
			s.T().Logf("%s: GMP execution on Solana: %s", userLabel, solanaRelayTxSig)
...
func (s *IbcEurekaSolanaTestSuite) Test_GMPSPLTokenTransferFromCosmos() {
	...
	s.Require().True(s.Run("Relay and Execute SPL Transfer on Solana", func() {
		...
		s.Require().True(s.Run("Execute on Solana", func() {
			unsignedSolanaTx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(relayTxBodyBz))
			s.Require().NoError(err)
...
func (s *IbcEurekaSolanaTestSuite) Test_GMPTimeoutFromSolana() {
	...
	s.Require().True(s.Run("Relay timeout back to Solana", func() {
		...
		s.Require().NoError(err)
		s.Require().NotEmpty(resp.Tx, "Timeout relay transaction should not be empty")
		...
		err = s.SolanaChain.WaitForTxConfirmation(sig)
		s.Require().NoError(err, "Timeout tx should be confirmed")
