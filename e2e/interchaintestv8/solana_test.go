...
func (s *IbcEurekaSolanaTestSuite) Test_Deploy() {
	...
	s.Require().True(s.Run("Verify ics07-svm-tendermint", func() {
		...
		s.Require().Equal(simd.Config().ChainID, clientState.V0.ChainId)
		s.Require().Equal(testvalues.DefaultTrustLevel.Denominator, clientState.V0.TrustLevelDenominator)
		s.Require().Equal(testvalues.DefaultTrustLevel.Numerator, clientState.V0.TrustLevelNumerator)
		s.Require().Equal(uint64(testvalues.DefaultTrustPeriod), clientState.V0.TrustingPeriod)
		s.Require().True(clientState.V0.UnbondingPeriod > clientState.V0.TrustingPeriod)
		s.Require().Equal(uint64(testvalues.DefaultMaxClockDrift), clientState.V0.MaxClockDrift)
		s.Require().Equal(uint64(1), clientState.V0.LatestHeight.RevisionNumber)
		s.Require().Equal(uint64(0), clientState.V0.FrozenHeight.RevisionNumber)
		s.Require().Equal(uint64(0), clientState.V0.FrozenHeight.RevisionHeight)
	}))
