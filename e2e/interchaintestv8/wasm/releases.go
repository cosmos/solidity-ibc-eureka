...
func GetLatestAttestorLightClientRelease() (Release, error) {
	releases, err := GetAllWasmAttestorLightClientReleases()
	if err != nil {
		return Release{}, err
	}
	...
	var latestRelease Release
	for _, release := range releases {
		if strings.HasPrefix(release.TagName, wasmAttestorLightClientTagPrefix) {
			latestRelease = release
			break
		}
	}
	if latestRelease.TagName == "" {
		return Release{}, errors.New("no release found with tag prefix " + wasmAttestorLightClientTagPrefix)
	}

	return latestRelease, nil
}

func GetAllWasmAttestorLightClientReleases() ([]Release, error) {
	resp, err := http.Get(releaseAPI)
	if err != nil {
		return nil, err
	}
	...
	// Filter releases to include only those with the attestor wasm light client tag prefix
	var filteredReleases []Release
	for _, release := range allReleases {
		if strings.HasPrefix(release.TagName, wasmAttestorLightClientTagPrefix) {
			filteredReleases = append(filteredReleases, release)
	...
	if len(filteredReleases) == 0 {
		return nil, errors.New("no attestor light client releases found")
	}

	return filteredReleases, nil
}

func GetAllAttestorLightClientReleasesFromVersion(version string) ([]Release, error) {
	if !semver.IsValid(version) {
		return nil, errors.New("invalid version format: " + version)
	}
	...
	var filteredReleases []Release
	for _, release := range releases {
		releaseSemver := strings.TrimPrefix(release.TagName, wasmAttestorLightClientTagPrefix)
		if !semver.IsValid(releaseSemver) {
			return nil, errors.New("invalid semver tag in release: " + release.TagName)
		}
