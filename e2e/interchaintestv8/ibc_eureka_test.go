...
func (s *IbcEurekaTestSuite) TestICS20TransferERC20TokenfromEthereumToCosmosAndBack() {
	...
	s.Require().True(s.Run("recvPacket on Cosmos chain", func() {
		...
		txResp, err := s.BroadcastMessages(ctx, simd, cosmosUserWallet, 200_000, &channeltypesv2.MsgRecvPacket{
			Packet:          packet,
	...
	s.Require().True(s.Run("acknowledgePacket on Ethereum", func() {
		...
		proofHeight, ucAndMemProof, err := operator.UpdateClientAndMembershipProof(
			uint64(trustedHeight), uint64(latestHeight), string(packetAckPath),
			"--trust-level", testvalues.DefaultTrustLevel.String(),
			"--trusting-period", strconv.Itoa(testvalues.DefaultTrustPeriod),
		)
		s.Require().NoError(err)
	...
	s.Require().True(s.Run("Transfer tokens back from Cosmos chain", func() {
		...
		// TODO: Replace with a proper parse from events as soon as it is available in ibc-go
		sequence := s.tmpHackToGetSequenceFromTxResp(txResp)
		value := transfertypes.FungibleTokenPacketData{
			Denom:    ibcCoin.Denom,
		...
		s.Require().True(s.Run("Verify balances on Cosmos chain", func() {
			...
			s.Require().Equal(denomOnCosmos.Path(), resp.Balance.Denom)
		}))
	}))
	...
	s.Require().True(s.Run("Receive packet on Ethereum", func() {
		...
		proofHeight, ucAndMemProof, err := operator.UpdateClientAndMembershipProof(
			uint64(trustedHeight), uint64(latestHeight), string(packetCommitmentPath),
			"--trust-level", testvalues.DefaultTrustLevel.String(),
			"--trusting-period", strconv.Itoa(testvalues.DefaultTrustPeriod),
		)
		s.Require().NoError(err)
	...
	s.Require().True(s.Run("Acknowledge packet on Cosmos chain", func() {
		...
		storageProofBz := s.getCommitmentProof(path)
		txResp, err := s.BroadcastMessages(ctx, simd, cosmosUserWallet, 200_000, &channeltypesv2.MsgAcknowledgement{
			Packet: returnPacket,
				...
				AcknowledgementResults: []channeltypesv2.AcknowledgementResult{
					{
						...
						RecvPacketResult: channeltypesv2.RecvPacketResult{
							Status:          0, // TODO: Enum or something?
...
// 		s.Require().NoError(err)
//
// 		packetCommitmentPath := ibchostv2.PacketCommitmentPath(sendPacket.SourcePort, sendPacket.SourceChannel, sendPacket.Sequence)
// 		proofHeight, ucAndMemProof, err := operator.UpdateClientAndMembershipProof(
...
//
// 		// This will be a membership proof since the acknowledgement is written
// 		packetAckPath := ibchostv2.PacketAcknowledgementPath(returnPacket.DestPort, returnPacket.DestChannel, uint64(returnPacket.Sequence))
// 		proofHeight, ucAndMemProof, err := operator.UpdateClientAndMembershipProof(
...
// 		packetReceiptPath := ibchostv2.PacketReceiptPath(packet.DestPort, packet.DestChannel, uint64(packet.Sequence))
// 		proofHeight, ucAndMemProof, err := operator.UpdateClientAndMembershipProof(
