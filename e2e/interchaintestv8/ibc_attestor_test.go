...
func (s *IbcAttestorTestSuite) SetupSuite(ctx context.Context, proofType types.SupportedProofType, chainType attestor.ChainType) {
	...
	attestorResult := attestor.SetupEthAttestors(ctx, s.T(), s.GetDockerClient(), s.GetNetworkID(), eth.DockerRPC, s.contractAddresses.Ics26Router, chainType)
...
func (s *IbcAttestorTestSuite) AggregatorStartUpTest(ctx context.Context, chainType attestor.ChainType) {
	...
	// Get the Anvil chain and its Docker RPC address
	anvil := chains[0].(*icfoundry.AnvilChain)
	// Use Docker internal address since attestor runs in Docker
	anvilRPC := anvil.GetRPCAddress()
	s.T().Logf("Anvil chain started, Docker RPC: %s", anvilRPC)
	s.T().Log("Setting up attestors...")
	// Use a placeholder router address
	dummyRouterAddress := "0x0000000000000000000000000000000000000001"
	_ = attestor.SetupAttestors(ctx, s.T(), attestor.SetupParams{
		NumAttestors:         testvalues.NumAttestors,
		KeystorePathTemplate: testvalues.AttestorKeystorePathTemplate,
		ChainType:            chainType,
		AdapterURL:           anvilRPC,
		RouterAddress:        dummyRouterAddress,
