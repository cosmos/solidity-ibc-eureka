...
import (
	...
	attestation_light_client "github.com/cosmos/solidity-ibc-eureka/packages/go-anchor/attestationlightclient"
...
const (
	// Number of attestors to use in attestation tests.
	// Max supported is 11 due to Solana transaction size limit (1232 bytes).
	attestationTestNumAttestors = 11
)
...
func (s *IbcSolanaAttestationTestSuite) SetupSuite(ctx context.Context) {
	...
	deployTasks := []e2esuite.ParallelTaskWithResult[solanago.PublicKey]{
		...
		},
		{
			...
			Run: func() (solanago.PublicKey, error) {
				keypairPath := fmt.Sprintf("%s/attestation_light_client-keypair.json", keypairDir)
				return s.Solana.Chain.DeploySolanaProgramAsync(ctx, "attestation_light_client", keypairPath, deployerPath)
			},
		},
		{
	...
	attestation_light_client.ProgramID = s.AttestationLightClientProgramID
	...
	cosmosAttestorResult := attestor.SetupAttestors(ctx, s.T(), attestor.SetupParams{
		NumAttestors:         attestationTestNumAttestors,
	...
	solanaAttestorResult := attestor.SetupSolanaAttestors(ctx, s.T(), s.GetDockerClient(), s.GetNetworkID(), testvalues.SolanaLocalnetRPC, ics26_router.ProgramID.String())
	...
	routerStateAccount, _ = solana.Ics26Router.RouterStatePDA(ics26_router.ProgramID)
	clientAccount, _ := solana.Ics26Router.ClientPDA(ics26_router.ProgramID, []byte(s.AttestationClientID))
	clientSequenceAccount, _ := solana.Ics26Router.ClientSequencePDA(ics26_router.ProgramID, []byte(s.AttestationClientID))
...
func (s *IbcSolanaAttestationTestSuite) initializeAttestationLightClient(ctx context.Context, clientID string) {
	...
	minRequiredSigs := uint8(attestationTestNumAttestors)

	clientStatePDA, _ := solana.AttestationLightClient.ClientPDA(attestation_light_client.ProgramID, []byte(clientID))
	appStatePDA, _ := solana.AttestationLightClient.AppStatePDA(attestation_light_client.ProgramID)

	heightBytes := make([]byte, 8)
	binary.LittleEndian.PutUint64(heightBytes, latestHeight)
	consensusStatePDA, _ := solana.AttestationLightClient.ConsensusStateWithAccountSeedPDA(
		attestation_light_client.ProgramID,
		clientStatePDA.Bytes(),
		heightBytes,
	)

	initInstruction, err := attestation_light_client.NewInitializeInstruction(
		clientID,
		latestHeight,
		attestorAddresses,
		minRequiredSigs,
...
func (s *IbcSolanaAttestationTestSuite) Test_Attestation_Deploy() {
	...
	s.Require().True(s.Run("Verify attestation light client on Solana", func() {
		clientStatePDA, _ := solana.AttestationLightClient.ClientPDA(attestation_light_client.ProgramID, []byte(s.AttestationClientID))
		...
		clientState, err := attestation_light_client.ParseAccount_AttestationLightClientTypesClientState(accountInfo.Value.Data.GetBinary())
		s.Require().NoError(err)

		s.Require().Equal(s.AttestationClientID, clientState.ClientId)
		s.Require().Equal(uint8(1), clientState.MinRequiredSigs)
...
func (s *IbcSolanaAttestationTestSuite) Test_Attestation_SolanaAttestorVerifyPacketCommitment() {
	...
	s.Require().True(s.Run("Send packet on Solana", func() {
		...
		client, _ := solana.Ics26Router.ClientPDA(ics26_router.ProgramID, []byte(s.AttestationClientID))
		clientSequence, _ := solana.Ics26Router.ClientSequencePDA(ics26_router.ProgramID, []byte(s.AttestationClientID))
		...
		packetCommitmentPDA, _ = solana.Ics26Router.PacketCommitmentPDA(ics26_router.ProgramID, []byte(s.AttestationClientID), namespacedSequenceBytes)
...
func (s *IbcSolanaAttestationTestSuite) Test_Attestation_UnauthorizedClientUpdate() {
	ctx := context.Background()
	s.SetupSuite(ctx)

	simd := s.Cosmos.Chains[0]

	unauthorizedWallet := solanago.NewWallet()
	_, err := s.Solana.Chain.FundUserWithRetry(ctx, unauthorizedWallet.PublicKey(), testvalues.InitialSolBalance, 5)
	s.Require().NoError(err)

	s.T().Logf("Created unauthorized wallet: %s", unauthorizedWallet.PublicKey().String())

	s.Require().True(s.Run("Attempt update client without RELAYER_ROLE", func() {
		resp, err := s.RelayerClient.UpdateClient(ctx, &relayertypes.UpdateClientRequest{
			SrcChain:    simd.Config().ChainID,
			DstChain:    testvalues.SolanaChainID,
			DstClientId: s.AttestationClientID,
		})
		s.Require().NoError(err, "Relayer should build update client tx")
		s.Require().NotEmpty(resp.Tx)

		unsignedSolanaTx, err := solanago.TransactionFromDecoder(bin.NewBinDecoder(resp.Tx))
		s.Require().NoError(err)

		_, err = s.Solana.Chain.SignAndBroadcastTxWithRetry(ctx, unsignedSolanaTx, rpc.CommitmentFinalized, unauthorizedWallet)
		s.Require().Error(err, "Update client should fail without RELAYER_ROLE")
		s.T().Logf("Update client correctly rejected for unauthorized wallet: %v", err)
	}))
}
