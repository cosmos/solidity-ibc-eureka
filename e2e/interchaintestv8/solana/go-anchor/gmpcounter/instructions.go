...
// Builds a "on_gmp_ack" instruction.
// Handle GMP acknowledgement callback (called by GMP program)
func NewOnGmpAckInstruction(
	// Params:
	successParam bool,
	dataParam []byte,
	...
	// Accounts:
	appStateAccount solanago.PublicKey,
	gmpCallerAccount solanago.PublicKey,
) (solanago.Instruction, error) {
	buf__ := new(bytes.Buffer)
	enc__ := binary.NewBorshEncoder(buf__)

	// Encode the instruction discriminator.
	err := enc__.WriteBytes(Instruction_OnGmpAck[:], false)
	if err != nil {
		return nil, fmt.Errorf("failed to write instruction discriminator: %w", err)
	}
	{
		// Serialize `successParam`:
		err = enc__.Encode(successParam)
		if err != nil {
			return nil, errors.NewField("successParam", err)
		}
		// Serialize `dataParam`:
	...
	// Add the accounts to the instruction.
	{
		// Account 0 "app_state": Writable, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(appStateAccount, true, false))
		// Account 1 "gmp_caller": Read-only, Signer, Required
		// The GMP program calling us
		accounts__.Append(solanago.NewAccountMeta(gmpCallerAccount, false, true))
	}
...
// Builds a "on_gmp_timeout" instruction.
// Handle GMP timeout callback (called by GMP program)
func NewOnGmpTimeoutInstruction(
	// Params:
	dataParam []byte,
	...
	// Encode the instruction discriminator.
	err := enc__.WriteBytes(Instruction_OnGmpTimeout[:], false)
	if err != nil {
		return nil, fmt.Errorf("failed to write instruction discriminator: %w", err)
	}
	{
