...
func (s *Solana) CreateAttestationLightClientALTAccounts(clientID string) []solana.PublicKey {
	appStatePDA, _ := AttestationLightClient.AppStatePDA(attestation_light_client.ProgramID)
	clientStatePDA, _ := AttestationLightClient.ClientPDA(attestation_light_client.ProgramID, []byte(clientID))
