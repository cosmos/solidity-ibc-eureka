...
contract SdkCoinTest is Test, IISdkCoinErrors {
     ...
     *
     */
    function test_convertAmountFromERC20() public view {
        ...
        for (uint256 i = 0; i < testCases.length; i++) {
            ...
            (uint64 convertedAmount, uint256 remainder) =
                SdkCoin.convertAmountFromERC20(address(testERC20), tc.amount);
    ...
    function test_ERC20MetadataToSdkCoin_ConvertAmount() public {
        ...
        for (uint256 i = 0; i < testCases.length; i++) {
            ...
            (uint64 convertedAmount, uint256 remainder) =
                SdkCoin.convertAmountFromERC20(address(mockERC20Metadata), tc.amount);
     ...
     * the
     *      conversion function `convertAmountToERC20` correctly calculates the equivalent ERC20 token amount.
     *
     * The test cases cover the following scenarios:
    ...
    function test_SdkCoinToERC20Metadata_ConvertAmount() public {
        ...
        for (uint256 i = 0; i < testCases.length; i++) {
            ...
            TestERC20Metadata mockERC20Metadata = new TestERC20Metadata(tc.decimals);
            uint256 convertedAmount = SdkCoin.convertAmountToERC20(address(mockERC20Metadata), tc.cosmosAmount);
     ...
     * parameters,
     * it calls either the `convertAmountFromERC20` or `convertAmountToERC20` function and verifies that
    ...
    function testRevertConditions() public {
        ...
        for (uint256 i = 0; i < testCases.length; i++) {
            ...
            if (tc.evmAmount != 0) {
                SdkCoin.convertAmountFromERC20(tc.tokenAddress, tc.evmAmount);
            } else {
                SdkCoin.convertAmountToERC20(tc.tokenAddress, tc.cosmosAmount);
            }
        }
    }
    ...
    function testInvariant_ERC20toSdkCoin_EqualDecimals(uint256 amount) public {
        ...
        (uint64 convertedAmount, uint256 remainder) =
            SdkCoin.convertAmountFromERC20(address(tokenAddress), amount);
    ...
    function testInvariant_SdkCoinToERC20(uint64 amount, uint8 decimals) public {
        ...
        uint256 convertedAmount = SdkCoin.convertAmountToERC20(address(tokenAddress), amount);
