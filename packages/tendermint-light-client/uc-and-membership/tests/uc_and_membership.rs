...
#[test]
fn test_uc_and_membership_invalid_protobuf_header() {
    let fixture = load_combined_happy_path_fixture();

    // Try to create context with malformed header - this should fail during setup
    let result = create_context_with_malformed_header(fixture);

    // Malformed protobuf should fail during fixture parsing/context setup
    assert!(
        result.is_none(),
        "Malformed protobuf header should fail during context setup"
    );
}
...
fn test_uc_and_membership_update_client_error_type() {
    ...
    let Some(ctx) = setup_test_context(expired_fixture) else {
        return;
    };
...
fn test_uc_and_membership_membership_error_type() {
    ...
    let Some(ctx) = create_context_with_empty_proof(membership_fixture) else {
        return;
    };
