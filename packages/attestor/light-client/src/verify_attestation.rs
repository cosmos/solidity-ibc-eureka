...
use crate::{client_state::ClientState, error::SolanaIBCError};
...
impl Verifyable {
    ...
    pub fn new(
        ...
        {
            return Err(SolanaIBCError::InvalidAttestedData {
...
pub(crate) fn verify_attesation(
    ...
    for (att_key, att_sig) in verifiable.pubkeys.iter().zip(&verifiable.signatures) {
        if let Some(lc_key) = client_state.pub_keys.iter().find(|k| k == &att_key) {
            ...
            return Err(SolanaIBCError::UnknownPublicKeySubmitted { pubkey: *att_key });
        }
    }
