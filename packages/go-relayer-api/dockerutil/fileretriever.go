...
func NewFileRetriever(log *zap.Logger, cli *client.Client, runLabelValuie string) *FileRetriever {
	return &FileRetriever{log: log, cli: cli, runLabelValue: runLabelValuie}
}
...
func (r *FileRetriever) SingleFileContent(ctx context.Context, volumeName, relPath string) ([]byte, error) {
	...
	containerName := fmt.Sprintf("%s-getfile-%d-%s", DockerPrefix, time.Now().UnixNano(), RandLowerCaseLetterString(5))
	...
	cc, err := r.cli.ContainerCreate(
		...
		&container.Config{
			...
			Labels: map[string]string{RunLabel: r.runLabelValue},
		},
