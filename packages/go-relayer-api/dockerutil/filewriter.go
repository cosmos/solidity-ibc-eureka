...
func NewFileWriter(log *zap.Logger, cli *client.Client, runLabelValue string) *FileWriter {
	return &FileWriter{log: log, cli: cli, runLabelValue: runLabelValue}
}
...
func (w *FileWriter) WriteFile(ctx context.Context, volumeName, relPath string, content []byte) error {
	...
	containerName := fmt.Sprintf("%s-writefile-%d-%s", DockerPrefix, time.Now().UnixNano(), RandLowerCaseLetterString(5))
	...
	cc, err := w.cli.ContainerCreate(
		...
		&container.Config{
			...
			Labels: map[string]string{RunLabel: w.runLabelValue},
		},
