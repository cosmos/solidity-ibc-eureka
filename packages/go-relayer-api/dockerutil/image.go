...
func NewImage(logger *zap.Logger, cli *client.Client, networkID string, runLabelValue string, repository, tag string) Image {
	if logger == nil {
	...
	if runLabelValue == "" {
		panic("runLabelValue cannot be empty")
	}
	if repository == "" {
	...
	i.log = logger.With(
		zap.String("image", i.Ref()),
		zap.String("run_label_value", runLabelValue),
	)
	return i
}
...
func (image Image) CreateContainer(ctx context.Context, containerName, hostName string, cmd []string, opts ContainerOptions) (string, error) {
	...
	cc, err := image.client.ContainerCreate(
		...
		&container.Config{
			...
			Labels: map[string]string{RunLabel: image.runLabelValue},
		},
...
func (image Image) Start(ctx context.Context, cmd []string, opts ContainerOptions) (*Container, error) {
	...
	var (
		containerName = SanitizeContainerName(image.runLabelValue + "-" + RandLowerCaseLetterString(6))
