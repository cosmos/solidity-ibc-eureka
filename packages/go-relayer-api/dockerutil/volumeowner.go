...
func SetVolumeOwner(ctx context.Context, opts VolumeOwnerOptions) error {
	...
	containerName := fmt.Sprintf("%s-volumeowner-%d-%s", DockerPrefix, time.Now().UnixNano(), RandLowerCaseLetterString(5))
	...
	cc, err := opts.Client.ContainerCreate(
		...
		&container.Config{
			...
			Labels: map[string]string{RunLabel: opts.RunLabelValue},
		},
