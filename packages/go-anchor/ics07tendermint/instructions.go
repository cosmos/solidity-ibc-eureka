...
func NewAssembleAndUpdateClientInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		// Account 4 "submitter": Writable, Non-signer, Required
...
func NewCleanupIncompleteUploadInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		accounts__.Append(solanago.NewAccountMeta(metadataAccount, true, false))
		// Account 2 "submitter_account": Writable, Signer, Required
...
func NewCreateMetadataInstruction(
	...
	// Encode the instruction discriminator.
	err := enc__.WriteBytes(Instruction_CreateMetadata[:], false)
	if err != nil {
		return nil, fmt.Errorf("failed to write instruction discriminator: %w", err)
	}
	{
		...
		// Serialize `totalChunksParam`:
		err = enc__.Encode(totalChunksParam)
		if err != nil {
			return nil, errors.NewField("totalChunksParam", err)
		}
		// Serialize `headerCommitmentParam`:
		err = enc__.Encode(headerCommitmentParam)
		if err != nil {
			return nil, errors.NewField("headerCommitmentParam", err)
		}
	}
	accounts__ := solanago.AccountMetaSlice{}
