...
**Note on Packet Receipts**: When Solana acts as the destination chain, it must store packet receipts in a provable way for non-membership proofs. This requires additional infrastructure not covered in this ADR.
...
pub struct RouterState {
    ...
    pub active_packets: BTreeSet<u64>, // Just sequence numbers of active packets
}
```

**State Transitions**:
- **Send packet**: Set bit to 1, store packet commitment hash
- **Receive acknowledgment**: Set bit to 0, remove commitment (packet finalized)
- **Timeout packet**: Verify commitment, set bit to 0, remove commitment
- **Check if sent**: Read bit (1 = yes and active, 0 = no or already finalized)
- **Window slide**: Archive commitments, compute merkle root, clear old commitments
...
5. **Merkle Root Only (No Individual Commitments)**:
   ...
   - **Rejected**: Cannot verify packet data during timeout before archival
   - **Rejected**: Allows relayers to fabricate packet data
   - **Decision**: Accept the 32 bytes per packet cost for security
