...
import { ICS20Escrow } from "./utils/ICS20Escrow.sol";
...
contract ICS20Transfer is IIBCApp, IICS20Transfer, IICS20Errors, Ownable, ReentrancyGuard {
    ...
    /// @notice Mapping of channel ID to escrow contract address
    mapping(string channelId => ICS20Escrow ics20EscrowContract) private _channelEscrowContracts;
    ...
    function onSendPacket(OnSendPacketCallback calldata msg_) external onlyOwner nonReentrant {
        ...
        (address erc20Address, bool senderChainIsSource) = getSendERC20AddressAndSource(msg_.packet, packetData);

        if (senderChainIsSource) {
            // transfer the tokens to the escrow address
            address ics20EscrowAddress = _getEscrowAddress(msg_.packet.sourceChannel);
            _transferFrom(sender, ics20EscrowAddress, erc20Address, packetData.amount);
        } else {
            // receiver chain is source: burn the vouchers
            IBCERC20(erc20Address).burn(sender, packetData.amount);
        }
    ...
    function onRecvPacket(OnRecvPacketCallback calldata msg_) external onlyOwner nonReentrant returns (bytes memory) {
        ...
        ICS20Lib.PacketDataJSON memory packetData = ICS20Lib.unmarshalJSON(msg_.packet.data);
        (address erc20Address, bool receiverChainIsSource) = getReceiveERC20AddressAndSource(msg_.packet, packetData);
        ...
        if (receiverChainIsSource) {
            address ics20EscrowAddress = _getEscrowAddress(msg_.packet.destChannel);
            _transferFrom(ics20EscrowAddress, receiver, erc20Address, packetData.amount);
        } else {
            // sender is source, so we mint vouchers
            IBCERC20(erc20Address).mint(receiver, packetData.amount);
        }
    ...
    function onAcknowledgementPacket(OnAcknowledgementPacketCallback calldata msg_) external onlyOwner nonReentrant {
        ...
        if (keccak256(msg_.acknowledgement) != ICS20Lib.KECCAK256_SUCCESSFUL_ACKNOWLEDGEMENT_JSON) {
            _refundTokens(msg_.packet, packetData);
        }
    ...
    /// @notice Get the escrow address for a channel ID
    /// @param channelId The channel ID
    /// @return The address of the escrow contract
    function _getEscrowAddress(string memory channelId) internal returns (address) {
        address ics20EscrowAddress = address(_channelEscrowContracts[channelId]);
        if (ics20EscrowAddress == address(0)) {
            // nothing exists, so we create new contract and register it in the mapping
            ICS20Escrow ics20EscrowContract = new ICS20Escrow(IICS20Transfer(address(this)));
            _channelEscrowContracts[channelId] = ics20EscrowContract;
            ics20EscrowAddress = address(ics20EscrowContract);
        }

        return ics20EscrowAddress;
    }
        ...
        private
    {
        ...
        (address erc20Address, bool senderChainIsSource) = getSendERC20AddressAndSource(packet, packetData);

        if (senderChainIsSource) {
            address ics20EscrowAddress = _getEscrowAddress(packet.sourceChannel);

            // transfer the tokens back to the sender from the escrow address
            _transferFrom(ics20EscrowAddress, refundee, erc20Address, packetData.amount);
        } else {
            // tokens had been burned when they were sent, so we must mint them back
            IBCERC20(erc20Address).mint(refundee, packetData.amount);
        }
    }
        ...
        returns (address, bool)
    {
        ...
        bool senderChainIsSource = !ICS20Lib.hasPrefix(denomBz, sourceDenomPrefix);
        ...
        return (erc20Address, senderChainIsSource);
    }
    ...
    /// @param packetData The unmarshalled packet data
    /// @return The ERC20 address for the token in the packetData
    /// @return Whether the receiver chain (i.e. us) of the packet is the source of the token
        ...
        returns (address, bool)
    {
        bytes memory denomBz = bytes(packetData.denom);
        bytes memory sourceDenomPrefix = ICS20Lib.getDenomPrefix(packet.sourcePort, packet.sourceChannel);
        bool receiverChainIsSource = ICS20Lib.hasPrefix(denomBz, sourceDenomPrefix);
        ...
        return (erc20Address, receiverChainIsSource);
    }

    /// @notice Finds a contract in the IBC denom mapping, or creates a new IBCERC20 contract
    ...
    function findOrCreateERC20Address(string memory fullDenomPath, string memory base) internal returns (address) {
        // check if denom already has an IBC denom registered contract
        string memory ibcDenom = ICS20Lib.toIBCDenom(fullDenomPath);
        address erc20Address = address(_ibcDenomContracts[ibcDenom]);
        if (erc20Address == address(0)) {
