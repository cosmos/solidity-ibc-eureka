...
impl Signer {
    ...
    pub fn sign(&self, signable_data: impl Signable) -> Result<Attestation, AttestorError> {
        let bytes = signable_data.to_abi_encoded_bytes()
        ...
        Ok(Attestation { height, timestamp, attested_data: bytes, signature: sig65, address: address.0.to_vec() })
    }
...
mod tests {
    ...
    fn test_signature_is_now_65_bytes() {
        let signer = Signer { signer: PrivateKeySigner::random() };
        ...
        assert!(v == 27 || v == 28 || v == 0 || v == 1, "v should be 27/28 or 0/1 for Ethereum compatibility");
    }
    ...
    fn test_65_byte_signature_with_recovery_id() {
        ...
        assert_eq!(recovered_address, expected_address, "recovered address should match signer's address");
