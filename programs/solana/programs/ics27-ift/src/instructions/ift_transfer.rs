...
mod tests {
    ...
    fn test_construct_evm_mint_call_max_amount() {
        ...
        // Check amount encoding (u64::MAX in big-endian)
        let amount_bytes = &payload[36..68];
    ...
    fn test_construct_evm_mint_call_invalid_hex() {
        let receiver = "0xnothex";
        let result = construct_evm_mint_call(receiver, 100);
        assert!(result.is_err());
    }

    #[test]
    fn test_construct_evm_mint_call_short_address() {
        // Short address should still work (left-padded)
        let receiver = "0xabcd";
        let payload = construct_evm_mint_call(receiver, 100).unwrap();
        assert_eq!(payload.len(), 68);
        // Should be heavily zero-padded
    ...
    fn test_construct_cosmos_mint_call() {
        let denom = "uatom";
        let receiver = "cosmos1abc123";
        let amount = 1_000_000u64;

        let payload = construct_cosmos_mint_call(denom, receiver, amount);
    ...
    #[test]
    fn test_construct_cosmos_mint_call_escapes() {
        // Test that special characters in denom/receiver work
        let denom = "ibc/ABC123";
        let receiver = "cosmos1xyz";
        let amount = 42u64;
