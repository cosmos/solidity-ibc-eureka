...
fn test_on_timeout_packet_direct_call_fails() {
    ...
    let mint_account = solana_sdk::account::Account {
        lamports: 1_000_000,
        data: vec![0; 82], // SPL mint size
        owner: anchor_spl::token::ID,
        executable: false,
        rent_epoch: 0,
    };

    // Create mock mint authority account
    ...
    // Create mock router program account
    ...
    // Create mock token program account
    ...
    // Should fail because it's called directly, not via CPI from router
    // The CPI validation will fail with InvalidCpiCaller
    assert!(
        result.program_result.is_err(),
        "on_timeout_packet should fail when called directly (not via CPI from router)"
    );
}
...
#[test]
fn test_on_timeout_packet_gmp_upstream_caller_passes_validation() {
    let mollusk = setup_mollusk();
    ...
    let (pending_pda, pending_bump) = get_pending_transfer_pda(&mint, client_id, sequence);
    let (mint_authority_pda, _) = get_mint_authority_pda(&mint);
    // GMP as caller (upstream caller validation)
    let (instructions_sysvar, instructions_account) =
        create_instructions_sysvar_account_with_caller(ics27_gmp::ID);
    let (system_program, system_account) = create_system_program_account();
    ...
    // Token account with wrong owner to trigger constraint error (not CPI error)
    let sender_token_account_pda = Pubkey::new_unique();
    let mut sender_token_data = vec![0u8; 165];
    sender_token_data[0..32].copy_from_slice(&mint.to_bytes());
    sender_token_data[32..64].copy_from_slice(&wrong_owner.to_bytes());
    let sender_token_account = solana_sdk::account::Account {
        lamports: 1_000_000,
        data: sender_token_data,
        owner: anchor_spl::token::ID,
    ...
    // Expect UninitializedAccount (mint account data is mock/uninitialized)
    // This proves GMP passes CPI validation - if it failed, we'd get:
    // - DirectCallNotAllowed (7010) for direct calls
    // - UnauthorizedGmp (7014) for unauthorized upstream callers
    let checks = vec![mollusk_svm::result::Check::err(
        solana_sdk::program_error::ProgramError::UninitializedAccount,
    )];
