...
use crate::state::{
    AccountState, GMPAcknowledgement, GMPAcknowledgementExt, GMPAppState, GMPPacketData,
    SolanaInstruction, SolanaInstructionExt,
};
...
pub fn on_recv_packet<'info>(
    ...
    let port_id = &ctx.accounts.app_state.port_id;

    let (expected_app_state_pda, _bump) =
        Pubkey::find_program_address(&[GMP_APP_STATE_SEED, port_id.as_bytes()], ctx.program_id);
    ...
    require!(
        msg.payload.dest_port == app_state.port_id,
...
mod tests {
    ...
    #[test]
    fn test_execute_with_nonce_protection_rejects_frozen_account() {
        let (_account_pda, bump) =
            AccountState::derive_address("cosmoshub-1", "cosmos1test", b"", &crate::ID).unwrap();
        ...
        let result: Result<String> =
            execute_with_nonce_protection(&mut account_state, current_time, || {
                Ok("should not execute".to_string())
            });

        assert!(result.is_err());
        assert_eq!(account_state.nonce, old_nonce);
    }
    ...
    fn test_on_recv_packet_unauthorized_router() {
        ...
        let accounts = vec![
            create_gmp_app_state_account(
                app_state_pda,
                correct_router, // State has correct router
                port_id,
                authority,
                app_state_bump,
                false, // not paused
    ...
    fn test_on_recv_packet_insufficient_accounts() {
        ...
        let instruction = SolanaInstructionSDK {
            ...
            accounts: vec![
                ...
                AccountMeta::new(payer, false),
                AccountMeta::new_readonly(system_program::ID, false),
                // Not providing remaining_accounts (account_state and target_program)!
            ],
            data: instruction_data.data(),
        };
    ...
    fn test_on_recv_packet_success_with_cpi() {
        ...
        let account_state = AccountState::try_deserialize(&mut &acc.data[8..])
