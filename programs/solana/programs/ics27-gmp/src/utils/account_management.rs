...
pub fn get_or_create_account<'a>(
    ...
    if account_info.data_is_empty() {
        // Create new account
        let account_size = 8 + AccountState::INIT_SPACE;
        let rent = Rent::get()?;
        ...
        let account_state = AccountState::try_deserialize(&mut &account_data[8..])?;
...
pub fn save_account_state(
    ...
    // Write discriminator
    account_data[0..8].copy_from_slice(AccountState::DISCRIMINATOR);

    // Serialize account state after discriminator
    account_state.try_serialize(&mut &mut account_data[8..])?;
...
// TODO: Add additional cross-chain address utilities:
// - Add validation for different blockchain address formats (Ethereum, Cosmos, etc.)
// - Add address normalization and checksumming utilities
