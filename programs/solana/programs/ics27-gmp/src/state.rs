...
/// Extension trait for `SolanaInstruction` with Solana-specific helper methods
pub trait SolanaInstructionExt {
    ...
    fn to_account_metas(&self) -> Result<Vec<AccountMeta>>;

    /// Extract program ID as Solana Pubkey
    fn get_program_id(&self) -> Result<Pubkey>;
}

impl SolanaInstructionExt for SolanaInstruction {
    fn try_from_slice(data: &[u8]) -> Result<Self> {
        use prost::Message;
...
/// Legacy execution payload structure (deprecated - use `SolanaInstruction`)
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Debug)]
pub struct ExecutionPayload {
    /// Instruction data for target program
    pub instruction_data: Vec<u8>,

    /// Account metas for the instruction
    pub accounts: Vec<ExecutionAccountMeta>,
}

impl ExecutionPayload {
    /// Parse execution payload from bytes
    pub fn try_from_slice(data: &[u8]) -> Result<Self> {
        Self::deserialize(&mut &data[..]).map_err(|_| GMPError::InvalidExecutionPayload.into())
    }

    /// Validate execution payload
    pub fn validate(&self) -> Result<()> {
        require!(!self.instruction_data.is_empty(), GMPError::EmptyPayload);
        require!(self.accounts.len() <= 32, GMPError::TooManyAccounts); // Reasonable limit
        Ok(())
    }
}

/// Account meta for execution
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Debug)]
pub struct ExecutionAccountMeta {
    pub pubkey: Pubkey,
    pub is_signer: bool,
    pub is_writable: bool,
}

/// Extension trait for `GMPAcknowledgement` with helper constructors
pub trait GMPAcknowledgementExt {
...
impl GMPAcknowledgementExt for GMPAcknowledgement {
    fn success(data: Vec<u8>) -> Self {
        Self {
            success: true,
            data,
...
// TODO: ADR Architecture enhancements:
// - Add compute budget estimation for SolanaInstruction execution
// - Implement instruction batching for gas optimization
// - Add instruction size validation and limits
// - Consider adding instruction priority/ordering mechanisms
