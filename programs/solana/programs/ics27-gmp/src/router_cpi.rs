...
pub fn send_packet_cpi<'a>(
    ...
    if client_sequence_data.len() >= 16 {
        ...
        let current_sequence = u64::from_le_bytes(sequence_bytes.try_into().unwrap());
        // Return the sequence that was just used (current - 1)
        Ok(current_sequence.saturating_sub(1))
    } else {
        // Fallback value (should not happen in normal operation)
        Ok(1)
    }
}
...
pub fn parse_packet_data_from_router_cpi(
    ...
    // Use generated GmpPacketData type from proto files
    use crate::proto::GmpPacketData;

    // Decode Protobuf payload from IBC packet
    let proto_data = GmpPacketData::decode(msg.payload.value.as_slice())
        .map_err(|_| GMPError::PacketDataParseError)?;
