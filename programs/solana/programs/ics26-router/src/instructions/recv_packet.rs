...
mod tests {
    ...
    #[test]
    fn test_recv_packet_app_returns_no_data() {
        // Test that recv_packet fails when IBC app doesn't call set_return_data
        let mut ctx = setup_recv_packet_test(true, 1000);

        // Create special payload that triggers mock app to NOT return data
        let special_payload = b"NO_RETURN_DATA".to_vec();

        // Create payload chunk with the special test data
        let (payload_chunk_pda, payload_chunk_account) = create_payload_chunk_account(
            ctx.accounts[0].0, // payer/submitter
            "client-1",
            ctx.packet.sequence,
            0,
            0,
        ...
        // Add the chunk account
        ctx.accounts
            .push((payload_chunk_pda, payload_chunk_account));

        // Update the instruction with metadata
        let msg = MsgRecvPacket {
            packet: ctx.packet.clone(),
            payloads: vec![PayloadMetadata {
                source_port: "source-port".to_string(),
        ...
        // Should fail with InvalidAppResponse because app didn't return data
        let checks = vec![Check::err(ProgramError::Custom(
            ANCHOR_ERROR_OFFSET + RouterError::InvalidAppResponse as u32,
        ))];
