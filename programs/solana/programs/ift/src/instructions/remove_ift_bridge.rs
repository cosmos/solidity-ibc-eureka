...
mod tests {
    ...
    #[test]
    fn test_remove_ift_bridge_fake_sysvar_attack_fails() {
        let mollusk = setup_mollusk();
        ...
        assert!(
            result.program_result.is_err(),
            "remove_ift_bridge should fail with fake sysvar attack"
        );
    }

    #[test]
    fn test_remove_ift_bridge_cpi_rejection_fails() {
        let mollusk = setup_mollusk();
        ...
        assert!(
            result.program_result.is_err(),
            "remove_ift_bridge should fail when called via CPI"
        );
    }
}
