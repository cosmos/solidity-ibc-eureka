...
mod tests {
    ...
    fn run_set_paused_error_test(case: SetPausedErrorCase) {
        let mollusk = setup_mollusk();
        ...
        let (use_unauthorized, sysvar_mode) = match case {
            SetPausedErrorCase::Unauthorized => (true, SysvarMode::Normal),
            SetPausedErrorCase::FakeSysvarAttack => (false, SysvarMode::FakeSysvar),
            SetPausedErrorCase::CpiRejection => (false, SysvarMode::CpiCall),
        };
    ...
    #[rstest]
    #[case::unauthorized(SetPausedErrorCase::Unauthorized)]
    #[case::fake_sysvar_attack(SetPausedErrorCase::FakeSysvarAttack)]
    #[case::cpi_rejection(SetPausedErrorCase::CpiRejection)]
    fn test_set_paused_validation(#[case] case: SetPausedErrorCase) {
        run_set_paused_error_test(case);
    }
}
