...
//!
//! Note: We use helper functions instead of TryFrom implementations to avoid
//! Rust's orphan rules (can't implement external traits for external types).
...
// ============================================================================
// Conversion functions: Borsh types → ibc-rs types (for deserialization)
// ============================================================================

pub fn borsh_to_height(bh: BorshHeight) -> Result<Height, &'static str> {
    Height::new(bh.revision_number, bh.revision_height)
...
pub fn borsh_to_public_key(bpk: BorshPublicKey) -> Result<PublicKey, &'static str> {
    match bpk {
        ...
        BorshPublicKey::Secp256k1(_) => {
            Err("Secp256k1 not supported on Solana")
        }
    }
}
...
pub fn borsh_to_validator_set(bvs: BorshValidatorSet) -> Result<ValidatorSet, &'static str> {
    let validators: Result<Vec<_>, _> = bvs.validators.into_iter()
        .map(borsh_to_validator)
        .collect();
...
pub fn borsh_to_commit(bc: BorshCommit) -> Result<Commit, &'static str> {
    let signatures: Result<Vec<_>, _> = bc.signatures.into_iter()
        .map(borsh_to_commit_sig)
        .collect();

    Ok(Commit {
        height: tendermint::block::Height::try_from(bc.height)
            .map_err(|_| "Invalid commit height")?,
        round: tendermint::block::Round::try_from(bc.round)
            .map_err(|_| "Invalid commit round")?,
        block_id: borsh_to_block_id(bc.block_id)?,
        signatures: signatures?,
    })
}

pub fn borsh_to_consensus_version(bcv: BorshConsensusVersion) -> tendermint::block::header::Version {
...
pub fn borsh_to_block_header(bbh: BorshBlockHeader) -> Result<TmHeader, &'static str> {
    ...
    let last_commit_hash = if let Some(h) = bbh.last_commit_hash {
        let hash_bytes: [u8; 32] = h.try_into()
    ...
    let data_hash = if let Some(h) = bbh.data_hash {
        let hash_bytes: [u8; 32] = h.try_into()
            .map_err(|_| "Invalid data hash length")?;
        Some(Hash::Sha256(hash_bytes))
    } else {
        None
    };
    ...
    let evidence_hash = if let Some(h) = bbh.evidence_hash {
        let hash_bytes: [u8; 32] = h.try_into()
            .map_err(|_| "Invalid evidence hash length")?;
        Some(Hash::Sha256(hash_bytes))
    } else {
        None
    };

    let address_bytes: [u8; 20] = bbh.proposer_address.try_into()
        .map_err(|_| "Invalid proposer address length")?;

    Ok(TmHeader {
        version: borsh_to_consensus_version(bbh.version),
        chain_id: bbh.chain_id.try_into()
        ...
        last_commit_hash,
        data_hash,
        validators_hash: Hash::Sha256(bbh.validators_hash.try_into()
            .map_err(|_| "Invalid validators hash length")?),
        next_validators_hash: Hash::Sha256(bbh.next_validators_hash.try_into()
            .map_err(|_| "Invalid next validators hash length")?),
        consensus_hash: Hash::Sha256(bbh.consensus_hash.try_into()
            .map_err(|_| "Invalid consensus hash length")?),
        app_hash: tendermint::AppHash::try_from(bbh.app_hash)
            .map_err(|_| "Invalid app hash")?,
        last_results_hash,
        evidence_hash,
...
pub fn borsh_to_signed_header(bsh: BorshSignedHeader) -> Result<SignedHeader, &'static str> {
    ...
    SignedHeader::new(header, commit)
        .map_err(|_| "Failed to create signed header")
}
