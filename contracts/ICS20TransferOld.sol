...
import { IICS20ErrorsOld } from "./errors/IICS20ErrorsOld.sol";
import { ICS20LibOld } from "./utils/ICS20LibOld.sol";
...
import { IICS20TransferOld } from "./interfaces/IICS20TransferOld.sol";
import { IICS26Router } from "./interfaces/IICS26Router.sol";
import { IICS26RouterMsgs } from "./msgs/IICS26RouterMsgs.sol";
import { IBCERC20Old } from "./utils/IBCERC20Old.sol";
    ...
    Multicall
{
    ...
    /// @param ibcDenomContracts Mapping of non-native denoms to their respective IBCERC20Old contracts
    /// @custom:storage-location erc7201:ibc.storage.ICS20Transfer
    struct ICS20TransferStorage {
        IEscrow escrow;
        mapping(string => IBCERC20Old) ibcDenomContracts;
    }
    ...
    {
        return ICS20LibOld.newMsgSendPacketV1(sender, msg_);
    }

    /// @inheritdoc IICS20TransferOld
    function sendTransfer(SendTransferMsg calldata msg_) external override returns (uint32) {
        return IICS26Router(owner()).sendPacket(ICS20LibOld.newMsgSendPacketV1(_msgSender(), msg_));
    }

    /// @inheritdoc IIBCApp
    function onSendPacket(OnSendPacketCallback calldata msg_) external onlyOwner nonReentrant {
        require(
            keccak256(bytes(msg_.payload.version)) == keccak256(bytes(ICS20LibOld.ICS20_VERSION)),
            ICS20UnexpectedVersion(ICS20LibOld.ICS20_VERSION, msg_.payload.version)
        );

        ICS20LibOld.FungibleTokenPacketData memory packetData =
            abi.decode(msg_.payload.value, (ICS20LibOld.FungibleTokenPacketData));
        ...
        address sender = ICS20LibOld.mustHexStringToAddress(packetData.sender);
        ...
        if (!originatorChainIsSource) {
            ...
            IBCERC20Old ibcERC20 = IBCERC20Old(erc20Address);
    ...
    function onRecvPacket(OnRecvPacketCallback calldata msg_) external onlyOwner nonReentrant returns (bytes memory) {
        ...
        if (keccak256(bytes(msg_.payload.version)) != keccak256(bytes(ICS20LibOld.ICS20_VERSION))) {
            // TODO: Figure out if should actually error out, or if just error acking is enough
            return ICS20LibOld.errorAck(abi.encodePacked("unexpected version: ", msg_.payload.version));
        }
        ...
        if (packetData.amount == 0) {
            return ICS20LibOld.errorAck("invalid amount: 0");
        }

        (address receiver, bool receiverConvertSuccess) = ICS20LibOld.hexStringToAddress(packetData.receiver);
        if (!receiverConvertSuccess) {
            return ICS20LibOld.errorAck(abi.encodePacked("invalid receiver: ", packetData.receiver));
        }
        ...
        if (originatorChainIsSource) {
            ...
            IBCERC20Old(erc20Address).mint(packetData.amount);
        }

        // transfer the tokens to the receiver
        _getEscrow().send(IERC20(erc20Address), receiver, packetData.amount);

        return ICS20LibOld.SUCCESSFUL_ACKNOWLEDGEMENT_JSON;
    }
    ...
    function onAcknowledgementPacket(OnAcknowledgementPacketCallback calldata msg_) external onlyOwner nonReentrant {
        ...
        if (keccak256(msg_.acknowledgement) != ICS20LibOld.KECCAK256_SUCCESSFUL_ACKNOWLEDGEMENT_JSON) {
    ...
    function _refundTokens(ICS20LibOld.FungibleTokenPacketData memory packetData, address erc20Address) private {
        address refundee = ICS20LibOld.mustHexStringToAddress(packetData.sender);
        ...
        returns (address, bool)
    {
        ...
        bytes memory sourceDenomPrefix = ICS20LibOld.getDenomPrefix(sourcePort, sourceChannel);
        bool originatorChainIsSource = !ICS20LibOld.hasPrefix(denomBz, sourceDenomPrefix);

        address erc20Address;
        if (originatorChainIsSource) {
            bool isERC20Address;
            (erc20Address, isERC20Address) = ICS20LibOld.hexStringToAddress(packetData.denom);
            if (!isERC20Address) {
                string memory ibcDenom = ICS20LibOld.toIBCDenom(packetData.denom);
        ...
        returns (address, bool)
    {
        ...
        if (originatorChainIsSource) {
            ...
            bytes memory newDenomPrefix = ICS20LibOld.getDenomPrefix(destPort, destChannel);
            ...
            erc20Address = ICS20LibOld.mustHexStringToAddress(erc20AddressStr);
        }

        return (erc20Address, originatorChainIsSource);
    }

    /// @notice Finds a contract in the foreign mapping, or creates a new IBCERC20Old contract
    ...
    /// @param base The base denom of the token, used when creating a new IBCERC20Old contract
    ...
    function findOrCreateERC20Address(string memory fullDenomPath, string memory base) internal returns (address) {
        ...
        string memory ibcDenom = ICS20LibOld.toIBCDenom(fullDenomPath);
        ...
        if (erc20Contract == address(0)) {
            ...
            IBCERC20Old ibcERC20 = new IBCERC20Old(this, _getEscrow(), ibcDenom, base, fullDenomPath);
